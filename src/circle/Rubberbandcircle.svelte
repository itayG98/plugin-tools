  ...
  const updateShape = (maybeEvent?: Event) => {
    ...
    if (origin && evt) {
      ...
      if (isCtrlPressed) {
        ...
        x = anchor[0] < origin[0] ? origin[0] - w : origin[0];
        y = anchor[1] < origin[1] ? origin[1] - h : origin[1];
      }
    }
...
<g 
  ...
  {#if origin}
    <defs>
      <mask id={maskId} class="a9s-rubberband-circle-mask">
        <rect 
          x={x - buffer} 
          y={y - buffer} 
          width={w + 2 * buffer}
...
<style>
  mask.a9s-rubberband-circle-mask > rect {
    fill: #fff;
  }

  mask.a9s-rubberband-circle-mask > circle {
    fill: #000;
  }
</style>